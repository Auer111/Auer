...
namespace Auer.Models
{
    ...
    public class NHTSAVehicleFullWrapper
    {
        public int Count { get; set; }
        public string Message { get; set; }
        public string SearchCriteria { get; set; }
        public IList<NHTSAVehicleFull> Results { get; set; }
    }
    public class NHTSAVehicleFull
    {
        public List<KVP> GetDataList()
        {
            List<KVP> dict = new List<KVP>();

            PropertyInfo[] properties = GetType().GetProperties();
            int i = 0;
            foreach (PropertyInfo pi in properties)
            {
                dict.Add(new KVP { Name = pi.Name, Value = pi.GetValue(this, null).ToString() });
                i++;
            }
            return dict;
        }
